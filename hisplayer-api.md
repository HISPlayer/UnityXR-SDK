...
* **public class StreamProperties**:
    ...
    * **public Transform xrLayerRenderTarget**: Property for external surface render mode. Attach the Unity Game Object's Transform where video will be rendered (Platforms: Android, OpenXR).
    * **public int xrLayerOrder**: Property for external surface render mode. Order of the rendered video relative to the whole Unity scene: 1 or higher will render the video over everything the camera renders. -1 or lower will render the video behind everything. Never put 0 which is the order of Unity's own Default Scene Layer. Set unique value across multiple streams to avoid multipe video rendering order conflict. Stereo layer takes two orders - this one and the next. (Platforms: Android, OpenXR)
    * **public bool xrLayerMatchVideoAspect**: Property for external surface render mode. Match the render surface quad to the video's aspect ratio. For example a 21:9 film is not stretched to fill a 16:9 quad. The quad never grows past the RenderTarget's scale. (Platforms: Android, OpenXR)
    * **public HISPlayerXRLayerProjection xrLayerProjection**: Property for external surface render mode. Select the projection type. Quad is a flat/rectilinear screen placed by the RenderTarget above. Equirect360 and Equirect180 wrap it around the viewer for 360/180 degree video. (Platforms: Android, OpenXR)
    * **public HISPlayerStereoMode xrLayerStereoMode**: Property for external surface render mode. Select stereoscopic mode. None for Monoscopic video. Left Right or Top Bottom for stereoscopic video. A stereo layer occupies two composition orders - the one above and the next one up. (Platforms: Android, OpenXR)
    * **public float xrLayerRadius**: Property for external surface render mode. Radius in metres of the equirect sphere. 0 is default for 360 video to make it infinite. (Platforms: Android, OpenXR)    
